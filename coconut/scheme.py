...
def agg_cred(params, sigs, threshold=True):
	...
	n = len(sigs)
	# evaluate all lagrange basis polynomial li(0)
	l = [lagrange_basis(n, o, i, 0) for i in range(1,n+1)] if threshold else [1 for _ in range(n)]
	# aggregate sigature
	(h, s) = zip(*sigs)
	aggr_s = ec_sum([l[i]*s[i] for i in range(n) if s[i] is not None)
	return (h[0], aggr_s)
